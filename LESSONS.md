...
- Showcase persona names are fixture metadata, not authorization roles. The frontend must render the backend-selected mode and server reasons, while the focused Profile C flow verifies Jazz/Rock/Soul dominance without recomputing scores or changing canonical state.
- Do not over-engineer, over-complicate, or over-test. Prefer one representative desktop showcase flow plus existing unit/contract coverage unless the changed boundary is responsive, accessible, or cross-browser.
