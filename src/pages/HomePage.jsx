...
const recommendationAriaLabel = () => 'Recommended records';
...
export default function HomePage() {
  ...
  const highlights = researchOnly
    ? [
          [String(catalog.meta?.total ?? 0), 'Records'],
    ...
    <main>
      <section className="hero" aria-labelledby="hero-heading">
        <div className="container hero-inner">
          <p className="hero-eyebrow">{researchOnly ? 'Groovehaus Vinyl Records' : 'Bangkok\'s finest record shop'}</p>
          <h1 className="hero-title" id="hero-heading">Find the record<br />that <span>finds you back.</span></h1>
          <p className="hero-sub">{researchOnly ? 'Explore the collection, save favorites, rate records, and discover your next listen.' : 'New arrivals, rare pressings, and explainable recommendations - all in one crate.'}</p>
      ...
      <div className="container home-content">
        <section aria-labelledby="new-arrivals-heading">
          <h2 className="section-heading" id="new-arrivals-heading">{researchOnly ? <>Featured records <small>Explore the collection</small></> : <>New arrivals <small>Just landed this week</small></>}</h2>
        ...
        <section aria-labelledby="rec-home-heading" className="rec-section">
          ...
          {recommendationStatus === 'success' && <RecScroll records={recommendations.slice(0, 5)} ariaLabel={recommendationAriaLabel()} surface="home" />}
        </section>
      </div>
    </main>
  );
