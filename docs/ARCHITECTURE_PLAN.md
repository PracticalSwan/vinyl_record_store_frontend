...
8. Product card, detail, recommendation, wishlist, and cart surfaces route artwork through `ProductImage`. It validates the public mapping, trusts the backend's explicit `localArtworkAvailable` flag for dataset rows, generation-guards image events, and owns the remote proxy -> local endpoint -> placeholder sequence. Accepted v2 mappings use their separate verified local bundle; unresolved rows never request a legacy image.
