...
Production is GitHub-linked Netlify at `https://groovehaus-store.netlify.app/`, built from the sole `master` branch. `VITE_API_BASE_URL=/` keeps browser requests same-origin and the Netlify redirect proxies `/api/*` to `https://groovehaus-api.netlify.app/`.
...
- Render artwork through one resilient `ProductImage` state machine. Strict dataset rows use verified local -> proxy -> placeholder; presentation-only supplemental mappings use proxy -> placeholder; unresolved dataset rows use placeholder; legacy/seed remains proxy -> local -> placeholder. The strict 208-file v3 bundle remains source evidence, while 1,124 supplemental mappings bring visible artwork to 1,300/2,259 without Amazon images or exact-pressing claims.
...
- The active MongoDB source contains 2,305 sealed products; the customer presentation overlay suppresses 46 duplicate display rows and exposes 2,259. Admin labels 2,305 as a source count. Dataset-managed rows remain CLI-managed/read-only. The three showcase customers now have canonical synthetic Jazz/Rock/Soul preference, rating, and wishlist state; historical Amazon pseudonyms never reach the client.
...
- NEXT-01 through NEXT-05 remain historical approved evidence. The backend historical benchmark measured random, positive-popularity, content, and one observed-only biased-MF candidate; biased MF was negative and remains offline-only. The explicitly authorized 2026-08-29 maintenance supersedes only the current classroom/production setup with MongoDB/v3 Profile C: frontend `/me`, profile-domain presentation, exact feedback, and tracking controls; backend preference, behavior, aggregate-popularity, and hybrid stages enabled. Committed defaults are unchanged; see `DEMO_PERSONALIZATION_RUNBOOK.md`.
