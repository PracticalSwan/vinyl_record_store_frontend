...
| FR-025 | Dataset products request unrelated legacy artwork or unreviewed Amazon images. | High | Trust only the backend `localArtworkAvailable` flag, keep legacy and v2 manifests separate, use Amazon images never, and send ambiguous/unresolved rows directly to the generic placeholder. | controlled |
...
| FR-027 | Historical readiness or pseudonymous data is presented as customer personalization. | High | Keep historical data backend-only, preserve current mode copy, label readiness as data preparation, and require separate authorization for PERS-03 through PERS-09. | controlled |
