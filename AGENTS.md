...
- PERS-00 through PERS-08 / FFP-09 through FFP-13 are implemented. `AuthProvider` resolves before `CatalogProvider`; Home and Recommendations use `GET /api/recommendations/me`, key state by the authenticated public subject, abort and generation-guard identity changes, omit anonymous IDs for signed-in requests, and render `cold-start`, `preference-profile`, `behavior-profile`, `popularity`, `personalized-hybrid`, or `anonymous-fallback` honestly. Preference, behavior, popularity, hybrid, and exact feedback controls remain default-off through the backend flags; ranking quality is not claimed.
...
- Historical Amazon input is backend-only and data-ready, not a quality result or customer profile. PERS-03 through PERS-08 are implemented behind default-off flags; PERS-09 remains deferred and is not part of this batch.
