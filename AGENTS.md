...
- DATA-00 through DATA-15 compatibility is implemented: the active MongoDB catalog has 2,305 source-derived products, dynamic genre/format facets, nullable metadata, non-purchasable unknown price/stock, placeholder-only dataset artwork, and Admin source/version/read-only row handling. The 116-record legacy fallback and exactly three showcase customers remain intact.
