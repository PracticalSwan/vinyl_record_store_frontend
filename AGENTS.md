...
- PERS-00 through PERS-05 / FFP-09 through FFP-11 are implemented. `AuthProvider` resolves before `CatalogProvider`; Home and Recommendations use `GET /api/recommendations/me`, key state by the authenticated public subject, abort and generation-guard identity changes, omit anonymous IDs for signed-in requests, and render `cold-start`, `preference-profile`, or `anonymous-fallback` honestly. Preference ranking and exact feedback controls remain default-off through the backend flags; ranking quality is not claimed.
- Corrected DATA-00 through DATA-15 v3 compatibility is implemented: 2,305 research-only products, controlled nonzero facets, explicit original/edition years, no commerce controls, accepted-art local fallback or placeholder, and Admin source/version/read-only handling. V2 is the immediate rollback release, V1 is the identity/legacy base, and the 116-record legacy fallback plus exactly three showcase customers remain intact.
- FFP-06 structured artwork is implemented through one `ProductImage` boundary with approved-host validation, stable layout, lazy/eager sizing, source attribution, and a deterministic remote proxy -> backend-confirmed local endpoint -> placeholder chain. The backend commits separate verified sets for all 116 legacy records and every accepted v3 match (208 files in the verified v3 release); the v2 rollback evidence pins the same stable set. The backend owns ingestion, local provenance, and offline evaluation; its report remains `insufficient-evidence` without quality metrics.
...
- Historical Amazon input is backend-only and data-ready, not a quality result or customer profile. PERS-03 through PERS-05 are implemented behind default-off flags; PERS-06 through PERS-09 remain deferred and require a separate explicit task.
...
- Recommendation copy must distinguish `demo-profile`, `content-similarity`, `cold-start`, `preference-profile`, and `anonymous-fallback` modes.
