...
- Corrected DATA-00 through DATA-15 v2 compatibility is implemented: 2,305 research-only products, controlled nonzero facets, explicit original/edition years, no commerce controls, accepted-art local fallback or placeholder, and Admin source/version/read-only handling. V1, the 116-record legacy fallback, and exactly three showcase customers remain intact.
- FFP-06 structured artwork is implemented through one `ProductImage` boundary with approved-host validation, stable layout, lazy/eager sizing, source attribution, and a deterministic remote proxy -> backend-confirmed local endpoint -> placeholder chain. The backend commits separate verified sets for all 116 legacy records and every accepted v2 match (208 files in the verified v2 release). The backend owns ingestion, local provenance, and offline evaluation; its report remains `insufficient-evidence` without quality metrics.
